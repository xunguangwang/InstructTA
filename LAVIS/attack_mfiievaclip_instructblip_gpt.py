...
if __name__ == "__main__":
    ...
    # folder_to_save = os.path.join(args.output, 'reasoning_mfiievaclip_instructblip_{}_gpt_{}_{}'.format(args.llm, args.clip_encoder.replace('/', ''), args.batch_size))
    # folder_to_save = os.path.join(args.output, 'mfiievaclip_instructblip_{}_{}_{}'.format(args.llm, args.clip_encoder.replace('/', ''), args.batch_size))
    ...
    for i, ((image_org, path), (image_tgt, _), txt_ins) in enumerate(zip(data_loader_imagenet, data_loader_img_target, data_loader_instruction)):
        ...
        # get tgt featutres
        with torch.no_grad():
            tgt_vis_embed = clip_model.encode_image(clip_preprocess(image_tgt))
            tgt_vis_embed = tgt_vis_embed / tgt_vis_embed.norm(dim=1, keepdim=True)
        ...
        for j in range(args.steps):
            ...
            embedding_sim = torch.mean(torch.sum(adv_image_features * tgt_vis_embed, dim=1))
